...
from core.agno.tools import Toolkit
from core.agno.tools.function import Function
from core.agno.utils.log import log_debug, log_error, log_info, log_warning
from core.agno.utils.mcp import get_entrypoint_for_tool
...
try:
    ...
    from core.agno.utils.mcp import ClientSession
    MCP_AVAILABLE = False

    # Create compatibility classes for missing MCP client components
    @dataclass
    class StdioServerParameters:
        """Compatibility class for StdioServerParameters"""
        command: str
        args: List[str] = None
    ...
    # Note: SSEClientParams and StreamableHTTPClientParams are defined later in the file
    ...
    async def stdio_client(server_params: StdioServerParameters):
        """Compatibility wrapper for stdio_client using our ClientSession"""
        ...
        class StdioClientContext:
            ...
            async def __aenter__(self):
                # Create a command list for our ClientSession
                command = [self.server_params.command] + self.server_params.args
                self.session = ClientSession(server_command=command)
                await self.session.initialize()
                # Return a tuple compatible with the expected interface
                return (None, None, self.session)  # read, write, session

            async def __aexit__(self, exc_type, exc_val, exc_tb):
                if self.session:
                    await self.session.close()

        return StdioClientContext(server_params)

    async def sse_client(**kwargs):
        """Compatibility wrapper for sse_client using our ClientSession"""

        class SSEClientContext:
            ...
            async def __aenter__(self):
                ...
                self.session = ClientSession(server_url=url)
                await self.session.initialize()
                return (None, None, self.session)  # read, write, session
        ...
        return SSEClientContext(**kwargs)

    async def streamablehttp_client(**kwargs):
        """Compatibility wrapper for streamablehttp_client using our ClientSession"""

        class StreamableHTTPClientContext:
            def __init__(self, **kwargs):
                self.params = kwargs
                self.session = None
            ...
            async def __aenter__(self):
                url = self.params.get('url')
                if not url:
                    raise ValueError("URL is required for Streamable HTTP client")
        ...
        return StreamableHTTPClientContext(**kwargs)
