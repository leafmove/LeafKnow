        ...
        try:
            if config.provider == "openai":
                ...
                if not LLAMACPP_AVAILABLE:
                    raise ImportError("llama.cpp支持不可用，请确保llama.cpp服务器正在运行")
                return LlamaCpp(
                    id=config.model_id,
                    api_key=config.api_key or "not-required",  # llama.cpp通常不需要API密钥
                    base_url=config.base_url,
                    temperature=config.temperature,
                    max_tokens=config.max_tokens,
                )
        ...
        while True:
            try:
                ...
                if ":" in user_input and not user_input.startswith(("http://", "https://", "ftp://")):
                    ...
                    # 尝试作为Llama.cpp模型添加
                    elif not found_model and ("llamacpp" in potential_model.lower() or "llama.cpp" in potential_model.lower()):
                        # 提取模型名称
                        if "llamacpp" in potential_model.lower():
                            llama_model_name = potential_model.replace("llamacpp", "").strip()
                        else:
                            llama_model_name = potential_model.replace("llama.cpp", "").strip()

                        if not llama_model_name:
                            llama_model_name = "local-model"

                        # 标准化模型名称
                        llama_model_name = llama_model_name.strip()
                        config_name = f"Llama.cpp {llama_model_name}"
                        ...
                        if existing_model:
                            ...
                            llama_config = ModelConfig(
                                ...
                                base_url="http://127.0.0.1:8080/v1",
                                is_local=True,
                                description=f"动态添加的Llama.cpp模型: {llama_model_name}"
                            )

                            if self.model_manager.add_model(llama_config):
                                found_model = llama_config
                                print(f"[OK] 已添加Llama.cpp模型: {config_name}")
                                print(f"[提示] 请确保Llama.cpp服务器运行在 http://127.0.0.1:8080")
                            else:
                                print(f"[WARN] 无法添加Llama.cpp模型: {llama_model_name}")
