...
import { CopyIcon, CircleXIcon, SearchIcon, RotateCcwIcon } from 'lucide-react'
...
export function AiSdkChat({
  ...
  const { messages, sendMessage, status, error, setMessages } = useChat({
    transport: new DefaultChatTransport({
      api: "http://127.0.0.1:60000/chat/agent-stream",
    }),
    ...
    if (currentSession?.scenario_id && !screenshotPreview) {
      try {
        ...
        if (screenshotPath) {
          ...
          try {
            const response = await fetch(`http://127.0.0.1:60000/file-screening/by-path-hash?file_path=${encodeURIComponent(screenshotPath)}`)
            if (response.ok) {
                          ...
                          if (part.mediaType?.startsWith('image/')) {
                            ...
                            return (
                              <div key={`${message.id}-${index}`} className="mt-2">
                                <img 
                                  src={`http://127.0.0.1:60000/image/thumbnail?file_path=${encodeURIComponent(actualPath || '')}&width=300&height=200`}
                                  alt={part.filename || 'Attached image'}
                                  className="max-w-xs max-h-48 rounded-lg border cursor-pointer"
                                  onClick={() => {
                                    // 点击时显示全尺寸图片
                                    openUrl(`http://127.0.0.1:60000/image/full?file_path=${encodeURIComponent(actualPath || '')}`);
                                  }}
                                  onError={(e) => {
              ...
              {selectedImage && (
                <div className="w-[300px] p-2 bg-muted/50 backdrop-blur-sm rounded-lg border shadow-lg">
                  ...
                  <div className="flex items-center gap-2">
                    <img 
                      src={`http://127.0.0.1:60000/image/thumbnail?file_path=${encodeURIComponent(selectedImage)}&width=48&height=48`}
                      alt="Preview"
        ...
        <PromptInput onSubmit={handleFormSubmit} className="relative">
          <PromptInputBody className="border-0 m-0 p-0">
            <PromptInputTextarea
              ...
              placeholder={selectedImage ? "Describe what you want to know about this image..." : "你想说啥?"}
            />
          </PromptInputBody>
